...
const StandardSeatLayoutBlock = ({ totalSeat }) => {
  ...
  const seatsPerRow = 4; // still 4 seats per row, walkway is "extra"
  const totalSeats = Number(totalSeat) || 0;

  // Helper to map seat index -> grid column (skip col 3 for walkway)
  const getGridColumn = (index) => {
    const pos = index % 4; // seat position in the row (0..3)
    if (pos < 2) return pos + 1; // left two seats -> col 1,2
    return pos + 2; // right two seats -> col 4,5 (skipping 3)
  };
  ...
  return (
    <div className="flex flex-col items-center p-3">
      <h2 className="text-title font-semibold mb-4 bg-pink">Standard</h2>

      {/* MAIN SEAT GRID */}
      <div className="bg-gray-100 border border-black p-4 md:p-4 rounded-t-[3rem] rounded-b-3xl shadow-xl grid grid-cols-5 gap-4 pt-10 pb-8 relative w-fit">
        {/* WALKWAY */}
        <div className="absolute  rotate-270 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 col-span-1 flex items-center justify-center text-gray-500">
          Walkway
        </div>
        ...
        {/* {Array.from({ length: totalSeats }).map((_, index) => {
          ...
          const isSold = bookedSeatData
            ? bookedSeatData.status === "SUCCESS"
            : false;
          const isBooked = bookedSeatData
            ? bookedSeatData.status === "BOOKED"
            : false;

          let seatColor = seatTypeColors[seatType]?.color || "bg-green-400";
          if (isSold) seatColor += " opacity-50";
          ...
          return (
            <div
              ...
              style={{
                gridRow: Math.floor(index / seatsPerRow) + 2,
                gridColumn: getGridColumn(index), // skip column 3
              }}
              onClick={() => {
                if (!isSold) {
                  if (isBooked) {
        ...
        {Array.from({ length: totalSeats }).map((_, index) => {
          ...
          const locked = isLocked(seatNumber); // 👈 check if seat is locked

          let seatColor = seatTypeColors[seatType]?.color || "bg-green-400";
          if (isSold || isBooked) seatColor += " opacity-50";
          if (locked) seatColor = "bg-red-600"; // 👈 red for locked seats

          return (
            <div
              key={seatNumber}
              className={`min-w-12 min-h-12 md:w-14 md:h-14 max-w-16 max-h-16 flex items-center justify-center rounded-lg select-none
        ${isSold ? "cursor-not-allowed pointer-events-none" : "cursor-pointer"}
        ${seatColor}`}
              style={{
                gridRow: Math.floor(index / seatsPerRow) + 2,
                gridColumn: getGridColumn(index), // skip column 3
...
export default StandardSeatLayoutBlock;
