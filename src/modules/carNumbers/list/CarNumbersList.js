...
import { setPaginate } from "../carNumbersSlice";
import { carNumbersService } from "../carNumbersService";



export const CarNumbersList = () => {
  const { carNumbers, paginateParams } = useSelector((state) => state.carNumbers);
  ...
  const [columns, setColumns] = useState(
    getData(carNumberPayload.columnsName) == null
      ? carNumberPayload.columns
      : getData(carNumberPayload.columnsName)
  );
  ...
  const reloadData = () => {
    ...
    dispatch(setPaginate(carNumberPayload.paginateParams));
  };

  const deleteData = async () => {
    setIsLoading(true);
    const result = await carNumbersService.destory(dispatch, selectedId);
  ...
  const loadingData = useCallback(async () => {
    const result = await carNumbersService.index(dispatch, paginateParams);
    if (result.status === 200) {
      console.log(result);
    ...
    setIsLoading(false);
    if (getData(carNumberPayload.columnsName) == null) {
      setData(carNumberPayload.columnsName, carNumberPayload.columns);
    }
  ...
  useEffect(() => {
    setData(carNumberPayload.columnsName, columns);
  }, [columns]);
                      ...
                      >
                        <Grid item xs={1}>
                          <TableCustomizeSetting
                            payload={carNumberPayload.columns}
                            columns={columns}
