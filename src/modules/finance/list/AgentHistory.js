...
import { setAgentPaginate } from "../SalesHistorySlice";
import { useParams } from "react-router-dom";
import IsSettledColor from "../../../shares/IsSettledColor";

export const AgentHistory = () => {
  const { salesHistories, agentPaginateParams, summary } = useSelector(
  ...
  const loadingData = useCallback(async () => {
    const result = await SalesHistoryService.agentList(
      dispatch,
      params.id,
      agentPaginateParams
    );
  ...
  }, [dispatch, params.id, agentPaginateParams]);
  ...
  const onFilterByDate = (e) => {
    let updateagentPaginateParams = { ...agentPaginateParams };

    updateagentPaginateParams.start_date = e.startDate
      ? e.startDate.toISOString().split("T")[0]
      : "";
    updateagentPaginateParams.end_date = e.endDate
      ? e.endDate.toISOString().split("T")[0]
      : "";

    dispatch(setDateFilter(e));
    dispatch(setAgentPaginate(updateagentPaginateParams));
  };
  ...
  const reloadData = () => {
    ...
    dispatch(setAgentPaginate(SalesHistoryPayload.agentPaginateParams));
  };
                ...
                switch (col.id) {
                  ...
                  case "seat":
                    ...
                    return `<td>${
                      seats.length
                        ? seats
                            .map((s) => `${seatMapping(Number(s.number))}`)
                            .join(", ")
                        : "No data"
                    }</td>`;
                  case "start_time":
          ...
          <div className="flex flex-col justify-start items-start md:flex-row md:items-center gap-5 flex-wrap md:max-w-[768px] xl:max-w-[1440px]">
            ...
            <div>
              ...
              <LocalizationProvider dateAdapter={AdapterDayjs}>
                <DatePicker
                  value={
                    agentPaginateParams.start_date
                      ? dayjs(agentPaginateParams.start_date)
                      : null
                  }
                  onChange={(date) =>
                    dispatch(
            ...
            <div>
              ...
              <LocalizationProvider dateAdapter={AdapterDayjs}>
                <DatePicker
                  value={
                    agentPaginateParams.end_date
                      ? dayjs(agentPaginateParams.end_date)
                      : null
                  }
                  onChange={(date) =>
                    dispatch(
            ...
            >
              ...
              {total !== 0 && (
                <TableBody>
                  ...
                  {emptyRows(
                    agentPaginateParams.page,
                    agentPaginateParams.rowsPerPage,
                    salesHistories
                  ) > 0 && (
            ...
            <TableRow>
              <TableCell>
                <TablePagination
                  ...
                  colSpan={3}
                  count={total}
                  rowsPerPage={agentPaginateParams.per_page}
                  page={agentPaginateParams ? agentPaginateParams.page - 1 : 0}
                  SelectProps={{
                    inputProps: {
