...
import { paymentHistoryPayload } from "../../paymentHistory/paymentHistoryPayload";
import { paymentHistoryService } from "../../paymentHistory/paymentHistoryService";
import { setPaginate } from "../../paymentHistory/paymentHistorySlice";
import { PermissionGate } from "../../../helpers/PermissionGate";

export const FinanceMemberList = () => {
  ...
  const loadingData = useCallback(async () => {
    ...
    const result = await paymentHistoryService.memberIndex(
      dispatch,
      paginateParams
    );
    if (result.status === 200) {
              ...
              case "seat":
                ...
                return `"${
                  seats.length
                    ? seats
                        .map(
                          (s) => `${seatMapping(s.number)}(${s.type})` // ✅ use seatMapping here
                        )
                        .join(", ")
                    : "No data"
                }"`;
                            ...
                            switch (column.id) {
                              ...
                              case "option":
                                return (
                                  <>
                                    {row["status"] === "PENDING" && (
                                      <PermissionGate permission="Finance_Member_Pending">
                                        <>
                                          <IconButton
                                            sx={{
                                              cursor: "pointer",
                                    ...
                                    {(row["status"] === "SUCCESS" ||
                                      row["status"] === "BLOCKED" ||
                                      row["status"] === "BOOKED") && (
                                      <PermissionGate permission="Finance_Member_Delete">
                                        <>
                                          <IconButton
                                            sx={{
                                              cursor: "pointer",
