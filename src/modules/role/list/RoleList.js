...
import { setPaginate } from "../roleSlice";
...
export const RoleList = () => {
  ...
  const [columns, setColumns] = useState(
    getData(rolePayload.columnsName) == null
      ? rolePayload.columns
      : getData(rolePayload.columnsName)
  );
  ...
  const reloadData = () => {
    ...
    dispatch(setPaginate(rolePayload.paginateParams));
  };

  const deleteData = async () => {
    setIsLoading(true);
    const result = await roleService.destory(dispatch, selectedId);
  ...
  const exportExcelData = async () => {
    await roleService.exportexcel(dispatch);
  };

  const exportExcelParamsData = async () => {
    await roleService.exportexcelparams(dispatch, paginateParams);
  };

  const exportPdfData = async () => {
    await roleService.exportpdf(dispatch);
  };

  const exportPdfParamsData = async () => {
    await roleService.exportpdfparams(dispatch, paginateParams);
  };
  ...
  const importData = async (e) => {
    ...
    const create = await roleService.import(formData, dispatch);
  ...
  const loadingData = useCallback(async () => {
    ...
    setIsLoading(false);
    if (getData(rolePayload.columnsName) == null) {
      setData(rolePayload.columnsName, rolePayload.columns);
    }
  ...
  useEffect(() => {
    setData(rolePayload.columnsName, columns);
  }, [columns]);
