...
import { setPaginate } from "../sparesSlice";
import { spareService } from "../sparesService";


export const SpareList = () => {
  const { spares, paginateParams } = useSelector((state) => state.spares);
  ...
  const [columns, setColumns] = useState(
    getData(sparePayload.columnsName) == null
      ? sparePayload.columns
      : getData(sparePayload.columnsName)
  );
  ...
  const reloadData = () => {
    ...
    dispatch(setPaginate(sparePayload.paginateParams));
  };

  const deleteData = async () => {
    setIsLoading(true);
    const result = await spareService.destory(dispatch, selectedId);
  ...
  const loadingData = useCallback(async () => {
    const result = await spareService.index(dispatch, paginateParams);
    if (result.status === 200) {
      console.log(result);
    ...
    setIsLoading(false);
    if (getData(sparePayload.columnsName) == null) {
      setData(sparePayload.columnsName, sparePayload.columns);
    }
  ...
  useEffect(() => {
    setData(sparePayload.columnsName, columns);
  }, [columns]);
